...
function page() {
  ...
  const deleteComment = (id) => {
    axios.delete(`http://localhost:8585/comment/delete/${id}`).then(res => {
    ...
    <>
      ...
      <div className=" p-3 sm:w-10/12 w-11/12 mx-auto ">
       ...
       cookie.accesToken ?
          ...
          <TextField onChange={e => setText(e.target.value)} value={text} className="w-10/12 "   placeholder="Add a comment ..." variant="standard"/>
          ...
          {comments.map((e) => {
            return(
              <div className="shadow_product p-3.5 flex flex-col gap-3 border-1 border-sky-200 bg-slate-100 py-4">
                 ...
                 <div className='flex justify-end text-sm italic opacity-90'>
                   {e.date.split('T')[0]}
                 </div>
